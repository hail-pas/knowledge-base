...
class CoordinateMapper:
    ...
    def _build_mapping(self):
        ...
        for page in self._parse_result.pages:
            ...
            self._page_boundaries.append((global_pos, global_pos + page_len))
    ...
    def global_to_page(self, global_index: int) -> TextPosition:
        ...
        # 如果索引超出范围，取最后一个位置
        last_end = self._page_boundaries[-1][1]
        if global_index >= last_end:
            # 尝试容错：可能在分隔符上，返回前一页的末尾
            if global_index < last_end + len(self._separator):
                last_page_num = len(self._page_boundaries)
                last_page_len = self._page_boundaries[-1][1] - self._page_boundaries[-1][0]
        ...
        for i, (start, end) in enumerate(self._page_boundaries):
            if start <= global_index < end:
                page_num = i + 1
                char_index = global_index - start
                return TextPosition(page_number=page_num, char_index=char_index)
    ...
    def page_to_global(self, position: TextPosition) -> int:
        ...
        Raises:
            ValueError: 页码超出有效范围
        """
        page_idx = position.page_number - 1
        if page_idx < 0 or page_idx >= len(self._page_boundaries):
            raise ValueError(f"Invalid page number: {position.page_number}")

        start, _ = self._page_boundaries[page_idx]
    ...
    def get_pages_for_range(self, start: int, end: int) -> list[int]:
        ...
        try:
            ...
            for page_num in range(start_pos.page_number, end_pos.page_number + 1):
                if 1 <= page_num <= len(self._page_boundaries):
                    pages.add(page_num)
