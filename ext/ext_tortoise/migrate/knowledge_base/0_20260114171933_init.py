...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    `execute_params` JSON NOT NULL COMMENT 'temporal 执行参数',
    `status` VARCHAR(20) NOT NULL COMMENT '状态' DEFAULT 'started',
...
CREATE TABLE IF NOT EXISTS `document` (
    ...
    `workflow_version` SMALLINT NOT NULL COMMENT '工作流版本' DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `workflow` (
    ...
    `deleted_at` BIGINT COMMENT '删除时间',
    `uid` CHAR(36) NOT NULL UNIQUE,
    `version` SMALLINT NOT NULL COMMENT '工作流版本' DEFAULT 1,
    ...
    UNIQUE KEY `uid_workflow_documen_117609` (`document_id`, `version`),
    CONSTRAINT `fk_workflow_document_4ab5a6a0` FOREIGN KEY (`document_id`) REFERENCES `document` (`id`) ON DELETE CASCADE,
