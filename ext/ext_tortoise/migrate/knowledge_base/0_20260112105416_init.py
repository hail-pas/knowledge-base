...
CREATE TABLE IF NOT EXISTS `embedding_model_config` (
    ...
    `type` VARCHAR(21) NOT NULL COMMENT '模型类型（openai/sentence_transformers等）',
    ...
    UNIQUE KEY `uid_embedding_m_name_ddb042` (`name`, `deleted_at`),
...
CREATE TABLE IF NOT EXISTS `indexing_config` (
    ...
    `updated_at` DATETIME(6) NOT NULL COMMENT '更新时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    `deleted_at` BIGINT COMMENT '删除时间',
    `name` VARCHAR(100) NOT NULL COMMENT '索引配置名称',
    `index_name` VARCHAR(255) NOT NULL COMMENT '索引名称/命名空间',
    `indexing_type` VARCHAR(6) NOT NULL COMMENT '索引类型（sparse/dense/hybrid）',
    `sparse_config` JSON COMMENT '稀疏索引配置（如BM25参数）',
    `dense_config` JSON COMMENT '稠密索引配置（如向量维度、度量方式）',
    `status` VARCHAR(8) NOT NULL COMMENT '索引状态' DEFAULT 'creating',
    `is_enabled` BOOL NOT NULL COMMENT '是否启用' DEFAULT 1,
    `description` LONGTEXT NOT NULL COMMENT '描述信息',
    `backend_config_id` BIGINT NOT NULL COMMENT '索引后端配置',
    `collection_id` BIGINT NOT NULL COMMENT '关联集合',
    `embedding_model_config_id` BIGINT COMMENT '关联的Embedding模型配置（稠密索引需要）',
    UNIQUE KEY `uid_indexing_co_collect_d06d6a` (`collection_id`, `index_name`, `deleted_at`),
    CONSTRAINT `fk_indexing_indexing_e3b573e1` FOREIGN KEY (`backend_config_id`) REFERENCES `indexing_backend_config` (`id`) ON DELETE RESTRICT,
    CONSTRAINT `fk_indexing_collecti_93fc3959` FOREIGN KEY (`collection_id`) REFERENCES `collection` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_indexing_embeddin_99b48580` FOREIGN KEY (`embedding_model_config_id`) REFERENCES `embedding_model_config` (`id`) ON DELETE SET NULL,
    KEY `idx_indexing_co_created_48dd05` (`created_at`),
    KEY `idx_indexing_co_deleted_5b7b15` (`deleted_at`),
    KEY `idx_indexing_co_collect_cd463b` (`collection_id`),
    KEY `idx_indexing_co_backend_b6c050` (`backend_config_id`),
    KEY `idx_indexing_co_indexin_04f79d` (`indexing_type`),
    KEY `idx_indexing_co_status_6bfc00` (`status`)
) CHARACTER SET utf8mb4 COMMENT='索引配置表';
CREATE TABLE IF NOT EXISTS `llm_model_config` (
    ...
    `deleted_at` BIGINT COMMENT '删除时间',
    `name` VARCHAR(100) NOT NULL COMMENT '模型配置名称',
    `type` VARCHAR(12) NOT NULL COMMENT '模型类型（openai/azure_openai/anthropic/gemini/ollama/custom等）',
    ...
    `is_default` BOOL NOT NULL COMMENT '是否默认模型' DEFAULT 0,
    `description` LONGTEXT NOT NULL COMMENT '描述信息',
    UNIQUE KEY `uid_llm_model_c_name_f10552` (`name`, `deleted_at`),
