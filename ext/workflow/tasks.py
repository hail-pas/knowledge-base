...
async def _schedule_workflow_start_async(
    ...
    支持两种调用方式：
    1. 创建新 workflow：传入 config、config_format、initial_inputs
    2. 使用现有 workflow：传入 workflow_uid，直接使用已存在的 workflow

    Args:
        config: 工作流配置（创建新 workflow 时必填）
        config_format: 配置格式
        initial_inputs: 初始输入
        use_async: 是否使用 Celery apply_async，False 表示直接调用（用于本地调试）
        workflow_uid: 已存在的 workflow UID（使用现有 workflow 时必填）
    ...
    # 使用 workflow_uid 作为 trace_id，如果是新创建的 workflow，在创建后获取 uid
    ...
    if workflow_uid:
        ...
        logger.info(f"Using existing workflow: {workflow.uid}, use_async={use_async}")
    else:
        # 创建新的 workflow
        if not config:
...
async def schedule_workflow_start(
    ...
    支持两种调用方式：
    1. 创建新 workflow：传入 config、config_format、initial_inputs
    2. 使用现有 workflow：传入 workflow_uid

    Args:
        config: 工作流配置（DAG 结构，创建新 workflow 时必填）
    ...
    Examples:
        # 方式 1: 创建新 workflow
        workflow_uid = await schedule_workflow_start(config=config, config_format="dict")

        # 方式 2: 使用现有 workflow
        workflow_uid = await schedule_workflow_start(workflow_uid="existing-uuid")
    """
    return await _schedule_workflow_start_async(config, config_format, initial_inputs, use_async, workflow_uid)
...
async def _schedule_workflow_resume_async(workflow_uid: uuid.UUID, use_async: bool = True) -> str:
    ...
    with logger.contextualize(trace_id=workflow_uid):
...
async def _schedule_activity_handoff_async(activity_uid: uuid.UUID, use_async: bool = True) -> str:
    """三类调度任务的异步实现
...
async def schedule_activity_handoff(activity_uid: uuid.UUID, use_async: bool = True) -> str:
    """
...
def schedule_activity_handoff_sync(activity_uid: uuid.UUID, use_async: bool = True) -> str:
    """
...
def _schedule_workflow_start_celery(
    ...
    支持两种调用方式：
    1. 创建新 workflow：传入 config
    2. 使用现有 workflow：传入 workflow_uid
    """
    return schedule_workflow_start_sync(config, config_format, initial_inputs, use_async, uuid.UUID(workflow_uid))
...
def schedule_workflow_resume_celery(
    ...
    """
    return schedule_workflow_resume_sync(uuid.UUID(workflow_uid), use_async)
...
def _schedule_activity_handoff_celery(
    ...
    """
    return schedule_activity_handoff_sync(uuid.UUID(activity_uid), use_async)
