...
class TestCollectionCRUD:
    ...
    async def test_create_collection(self, client):
        """测试创建 Collection"""
        from .conftest import _generate_unique_id
        ...
        response = await client.post("/v1/service/document/collection", json=collection_data)
    ...
    async def test_list_collections(self, client):
        ...
        await client.post("/v1/service/document/collection", json={
        ...
        # 获取列表
        response = await client.get("/v1/service/document/collection")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0
        assert "data" in data
    ...
    async def test_get_collection_detail(self, client):
        ...
        # 从列表中获取刚创建的记录
        list_response = await client.get("/v1/service/document/collection")
        ...
        # 获取详情
        response = await client.get(f"/v1/service/document/collection/{pk}")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0
    ...
    async def test_update_collection(self, client):
        ...
        # 创建集合
        await client.post("/v1/service/document/collection", json=collection_data)
        ...
        response = await client.put(f"/v1/service/document/collection/{pk}", json=update_data)
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0

        # 验证更新
        detail_response = await client.get(f"/v1/service/document/collection/{pk}")
    ...
    async def test_delete_collection(self, client):
        ...
        # 删除集合
        response = await client.delete(f"/v1/service/document/collection/{pk}")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0
    ...
    async def test_filter_collections(self, client):
        ...
        # 过滤 is_public
        response = await client.get("/v1/service/document/collection?is_public=true")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0
        ...
        # 过滤 name
        response = await client.get("/v1/service/document/collection?name__icontains=public")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0
        items = data["data"]["items"]
    ...
    async def test_full_crud_flow(self, client):
        ...
        # 1. 创建
        create_response = await client.post("/v1/service/document/collection", json=collection_data)
        ...
        update_response = await client.put(
            f"/v1/service/document/collection/{pk}",
            json={"description": "Full flow collection update", "is_temp": True}
        )
        assert update_response.status_code == 200
        assert update_response.json()["code"] == 0

        # 验证更新
        updated_detail = await client.get(f"/v1/service/document/collection/{pk}")
        assert updated_detail.json()["data"]["description"] == "Full flow collection update"
        assert updated_detail.json()["data"]["is_temp"] is True

        # 5. 删除
        delete_response = await client.delete(f"/v1/service/document/collection/{pk}")
        assert delete_response.status_code == 200
        assert delete_response.json()["code"] == 0

        # 验证删除
        final_detail = await client.get(f"/v1/service/document/collection/{pk}")
    ...
    async def test_search_collections(self, client):
        ...
        # 搜索 "document"
        response = await client.get("/v1/service/document/collection?search=document")
        assert response.status_code == 200
        data = response.json()
        assert data["code"] == 0
        items = data["data"]["items"]
